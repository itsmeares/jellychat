...
Publish the v0.2.1 release output:
...
zip -r Jellyfin.Plugin.JellyChat_0.2.1.zip .
```

Create a GitHub release tagged `v0.2.1` and attach `Jellyfin.Plugin.JellyChat_0.2.1.zip`. The release workflow updates `manifest.json` with the release asset URL, checksum, timestamp, and version entry after the release is published.
